...
const AREA_ORDER = ['Rinologia', 'Oncologia', 'Otologia', 'Laringologia', 'Sono', 'Geral'];
const AREA_EMOJI: Record<string, string> = {
  Rinologia: '👃', Oncologia: '🔬', Otologia: '🦻', Laringologia: '🗣️', Sono: '😴', Geral: '⚕️'
};
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans text-slate-900">
      <header className="bg-[#00A0AF] text-white p-4 md:px-10 shadow-md z-10 relative">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="text-center sm:text-left">
            ...
            <p className="text-[#5CC6BA] font-medium mt-0.5 text-sm">12 Instrumentos Clínicos em Otorrinolaringologia</p>
          </div>
      ...
      <main className="flex-grow p-2 sm:p-6 max-w-7xl mx-auto w-full flex flex-col">
        ...
        <div className="w-full mb-6 space-y-2">
          {AREA_ORDER.map(area => {
            ...
            return (
              <div key={area} className="flex flex-wrap items-center gap-1">
                <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest w-full mb-0.5">
      ...
      <footer className="bg-white border-t p-4 text-center text-slate-400 text-xs mt-auto shadow-inner">
        © 2026 OTTO Triagem | 12 Instrumentos PROMs Otorrinolaringológicos Validados
      </footer>
    </div>
  );
}
