...
import { VOISS_QUESTIONS, VOISS_OPTIONS, getVoiSSClassification, REFERENCE_VOISS } from './logic';

interface Props { patientId: string; doctorId?: string; }

export default function VoiceVoiSS({ patientId, doctorId }: Props) {
  ...
  const handleSubmit = async () => {
    const cl = getVoiSSClassification(score);
    setResult({ score, ...cl });
    try {
      await fetch(`${API_BASE_URL}/api/results`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ patient_id: patientId || 'anon_voiss', doctor_id: doctorId || null, calc_type: 'voiss', score, raw_answers: answers, hub_version: '1.4.0' })
      });
    } catch {}
  };
  ...
  if (result) {
    return (
      <div className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-lg mt-4 text-center border-t-8" style={{ borderColor: result.color }}>
        <h2 className="text-3xl font-extrabold mb-1" style={{ color: result.color }}>VoiSS: {result.score} Pontos</h2>
        ...
        <div className="mt-6 bg-slate-50 p-4 rounded-lg border border-slate-200 text-xs text-slate-500 text-left">
          ...
          <p className="italic">{REFERENCE_VOISS}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto flex flex-col gap-6">
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg border border-slate-100">
        <div className="mb-8 border-b border-slate-200 pb-6">
          <h2 className="text-2xl font-bold text-slate-800 mb-1">VoiSS — Escala de Sintomas Vocais</h2>
          <p className="text-slate-500 text-sm mb-2">Voice Symptom Scale. Ferramenta clínica altamente sensível para isolar a disfunção glótica de alterações laríngeas ou de esforço puro.</p>
        ...
        <div className="flex justify-center border-t border-slate-200 pt-6">
          <button disabled={totalAnswered < VOISS_QUESTIONS.length} onClick={handleSubmit}
