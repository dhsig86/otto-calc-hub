export function getCentorClassification(score: number): { label: string; color: string; recommendation: string } {
  ...
  return { label: 'Até 68% de Hipótese (Muito Alta)', color: '#dc2626', recommendation: 'Elevada suspeita patológica orgânica (Sensibilidade máxima de ~68% para strep). Realizar cultura e tratar se positivo na base probabilística (Antibioticoterapia empírica considerável na demora de swabs).' };
}
